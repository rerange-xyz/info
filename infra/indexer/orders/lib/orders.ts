...
function getFromBlock(latestBlock: bigint, lookbackHours: number, averageBlockTimeSeconds: number) {
...
function uniqueBlockNumbers(logs: Array<{ blockNumber: bigint }>) {
	return [...new Set(logs.map((log) => log.blockNumber.toString()))].map((value) => BigInt(value))
}
