...
import { decodePoolAddressFromAdapterData, resolveAdapterKind } from "./adapter.js"
...
const rerangeFunctionAbiItem = (hubAbi as Abi).find(
	(item): item is AbiFunction =>
		item.type === "function" && item.name === "rerange" && item.inputs.length === 1,
)
...
function buildCandidate(
	...
	const readinessScore = triggerTicks > 0 ? tickDistance / triggerTicks : tickDistance
	const isLikelyReady =
...
async function loadCandidatesForChain(
	...
	const parsed = rows
		.map((row) => {
			...
			return state
				? buildCandidate(row, state, chain.deployment, config.distanceMultiplier)
				: null
		})
		.filter((candidate): candidate is ParsedOrderCandidate => Boolean(candidate))
		.sort((left, right) => {
	...
	const pools = await fetchPoolRowsByAddress({
		...
		chainId: chain.chainId,
		poolAddresses: [...new Set(parsed.map((candidate) => candidate.poolAddress))],
	})
	...
	return parsed
		...
		.filter(
			(candidate): candidate is ParsedOrderCandidate & { poolRow: PoolsTableRow } =>
				Boolean(candidate.poolRow),
		)
}

function decodeRerangePreview(resultData: Hex): DecodedRerangePreview {
	const [decoded] = decodeAbiParameters(rerangeFunctionOutputs, resultData) as [DecodedRerangePreview]
	return decoded
}
...
function rewardEthWeiFromPool(params: {
	...
	const token0DerivedEthWei = parseDecimalToWei(asString(token0?.derivedETH) ?? undefined)
	const token1DerivedEthWei = parseDecimalToWei(asString(token1?.derivedETH) ?? undefined)
	...
	if (
		...
		if (params.preview.reward1 > 0n && token1Decimals !== null && token1DerivedEthWei > 0n) {
			rewardEthWei +=
...
async function simulateBatch(
	...
	if (!chain.resolverAddress) {
		throw new Error(`Resolver private key is required for chain ${chain.chainId}`)
	}
	...
	const poolByKey = new Map(candidates.map((candidate) => [candidate.orderKey.toLowerCase(), candidate]))
	...
	for (const chain of config.chains) {
		...
		const successfulCandidates = candidates.filter((candidate) =>
			initialSimulation.decodedRewards.some(
				(reward) => reward.orderKey.toLowerCase() === candidate.orderKey.toLowerCase(),
			),
		)
		...
		if (profitableRewards.length === 0) {
			console.log(`Successful reranges on ${chain.chainKey} have zero indexed ETH reward.`)
			continue
		}
		...
		const gasEstimate = await chain.publicClient.estimateContractGas(finalRequest)
		...
		if (!gasPriceWei) {
			throw new Error(`Unable to estimate gas price for chain ${chain.chainId}`)
		}
		...
		if (config.dryRun) {
			console.log(`Dry run: profitable batch on ${chain.chainKey} not submitted.`)
			continue
		}

		if (!chain.walletClient || !chain.resolverAddress) {
			throw new Error(`Wallet client is required to execute on chain ${chain.chainId}`)
		}
