...
function parseBigIntValue(value: string | undefined, fallback: bigint) {
  ...
  try {
    ...
    throw new Error(
      `Expected a non-negative bigint string, received: ${value}`,
    );
  }
}
...
function getDeploymentByChainId(
  chainId: number,
): PublicDeploymentConfig | null {
...
function normalizeDeployment(params: {
  ...
  return {
    ...
    v3AdapterAddress: params.deployment.contracts
      .uniswapV3Adapter as `0x${string}`,
...
export function resolveResolverConfig(): ResolverConfig {
  ...
  const privateKey = process.env.RERANGE_RESOLVER_KEY?.trim() as
    | `0x${string}`
    | undefined;
  ...
  return {
    ...
    batchSize: parsePositiveInt(
      process.env.RESOLVER_BATCH_SIZE,
      DEFAULT_BATCH_SIZE,
    ),
